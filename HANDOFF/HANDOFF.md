...
### 本轮任务
新增第 13 篇技术文档：ERPNext 多公司与多站点管理实战

### 完成内容
1. **创建 `docs/erpnext/13_Multi_Company_Setup.md`**：
   - 多公司场景（集团管控/子公司独立核算/共享服务）
   - 多公司配置（公司树/内部交易配置示例/合并报表）
   - 多站点架构（bench new-site 命令/独立数据库/资源隔离对照表）
   - 权限管理（用户权限配置/角色继承/双重隔离方案）
   - 公司间内部交易流程（采购销售联动/资金调拨日记账）
   - 合并报表与内部消除（生成步骤/消除科目建议）

2. **更新 `mkdocs.yml`**：在「实施踩坑与配置指南」下新增「13 多公司与多站点管理」导航项

3. **验证**：`./venv/bin/mkdocs build --strict` 通过，exit code 0

4. **提交**：commit `3d7a682`，已推送至 origin/main

### 当前负责人
- Agent: CTO
- 完成时间: 2026-03-16 08:11 GMT+8
...
### 本轮任务
新增第 15 篇技术文档：ERPNext 工作流与审批配置指南

### 完成内容

1. **创建 `docs/erpnext/15_Workflow_and_Approval.md`**：
   - 标题：ERPNext 工作流与审批配置实战
   - 涵盖工作流基础（状态机/转换条件/动作）、多级/并行/条件审批配置、四大常见场景（采购/费用报销/请假/合同）、邮件通知（审批提醒/抄送/拒绝通知模板）、移动端审批（企业微信/钉钉 Webhook 集成）、审批记录与合规审计
   - 含配置截图说明、Jinja2 邮件模板示例、Python 条件表达式示例

2. **更新 `mkdocs.yml`**：在「实施踩坑与配置指南」下新增「15 工作流与审批配置」导航项

3. **验证**：`./venv/bin/mkdocs build --strict` 通过，exit code 0，0 warning

4. **提交**：commit `85619e5`，已推送至 origin/main
